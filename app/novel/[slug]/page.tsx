...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const chapterTitle = data.title || `Ch. ${currentChapterNum}`
  
  // Concise layout to keep overall title length under 70 characters for search engines
  let pageTitle = `${novel.title} ${chapterTitle} Free Read`
  if (pageTitle.length > 65) {
    pageTitle = `${novel.title.substring(0, 30)}... Ch. ${currentChapterNum}`
  }

  const description = `Read ${novel.title} - ${chapterTitle} online for free. Explore more chapters on Web Novel Reader.`
...
export default async function ChapterReaderPage({ params }: PageProps) {
  ...
  // 1. Fetch novel ID and title securely using slug
  ...
  // 3. Fire-and-forget background analytics/history updates
